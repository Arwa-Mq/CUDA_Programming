#include <cublas_v2.h> // including the lib cuBLAS
#include <cuda.h>
...
int main(void){
    ...
    // This describes the environment in which our cuBLAS routines are executed. It is initialised with cublasCreate().
    ...
    // To get an accurate timing measurement we will run the same code multiple times. Five times should be more than enough.
    int num_reps = 5;
    ...
    // Please read  https://docs.nvidia.com/cuda/cublas/index.html#cublas-t-gemm to understand the input parameters to cublasSgemm().
